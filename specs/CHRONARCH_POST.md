...
|---|---|---|
| **SpaceSeal** | a `PlotCommitment` — reserved space sealed to a farmer, `space_units` + optional `cas_root`; persists on disk as a `.cseal` ([SPACEFILE.md](SPACEFILE.md)) and a node farms from it ([FARMER.md](FARMER.md)) | `make_space_seal`, `verify_space_seal`, `write_space_seal`, `read_space_seal` |
