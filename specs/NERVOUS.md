...
Ids and names are verbatim from the kernel constants (`INTERFACES`, K7). Restriction at any interface is measured in basis points; a Scar object must name one of these ids (its schema rejects anything else). The "strain tends to transmit to" column records the model's standing one-hop predictions — verbatim from `ADJACENCY` in `chronarch-nervous`, with strain magnitude halving per hop. These are exactly the predictions the epoch loop tests; a failed prediction falsifies the model and is itself sealed as a scar (G18).

| Id | Name | What is measured | What restriction looks like | Strain tends to transmit to |
|---|---|---|---|---|
| I1 | `hash_walk` | Hash-link continuity over ring ranges (`HASH_WALK`); the walk from any ring back toward Ring 0 verifies. | A link that does not verify; a walk that cannot complete; a forged or mutated past ring (G1). | I3 (walks stall without retrievable objects), I5 (replay loses trusted inputs). |
| I2 | `plot_challenge_honesty` | PLOT LANE proof-of-space responses: plots answer slot challenges honestly and on time. | Missed or dishonest plot proofs; claimed space that cannot prove itself. | I1 (dishonest plots strain the clock the walk rides on), I9 (farmer rewards and bonds distort, [TOKEN.md](TOKEN.md)). |
| I3 | `cas_retrieval` | CAMBIUM/CAS LANE availability: pinned objects fetch and their bytes hash to the pin (`PIN_FETCH`, `PIN_VERIFY`). A pin failure is a nervous event, not a lost file. | Withheld pins; bytes that do not hash to the pin; retrieval latency collapse. | I4 (challenges cannot fetch their evidence), I5 (replay inputs missing). |
| I4 | `poq_challenge_pass_rate` | Fraction of PoQ challenges passed **by attestation** — advisory self-scores never count (G10, G6). | Falling pass rate; unanswered challenges; a challenge gap exceeding the cadence floor (Section 9). | I5 (unverified faculties keep running), I10 (councilors below cadence lose eligibility). |
| I5 | `faculty_replay` | Deterministic replay fidelity: live-registry faculties reproduce identical outputs (G3). | Divergent replay outputs; a faculty whose hash is not in the live registry executing anyway. | I4 (challenge machinery itself degrades), I6 (divergent replay invites injection). |
| I6 | `mempool_injection` | Admission screening of tx/config payloads against `FORBIDDEN_KEY_TOKENS` (`SCREEN_INJECTION`, K18). | Injection attempts; any payload carrying `admin_key`, `helm_override`, or kin ([GENESIS.md](GENESIS.md), Section 8). | I8 (a screen bypass is covenant drift). |
