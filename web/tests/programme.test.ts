...
describe("fixtures", () => {
  ...
  it("the synthesis child is legal: parents from both programmes, a live three-bridge path, a grant", () => {
    const r = validateChild(cat, CHILD);
...
describe("refusals are hard errors", () => {
  ...
  it("NO_BRIDGE: a missing, broken, draft or undeclared connection", () => {
    ...
    expect(() => validateChild(cat, missing)).toThrow(/NO_BRIDGE/);
    const none = legal();
    delete none.path;
    expect(() => validateChild(cat, none)).toThrow(/NO_BRIDGE.*no implicit coupling/);
  ...
  it("CROSS_SECTOR_WRITE: a child never writes into another sector's field", () => {
    ...
    expect(() => validateChild(cat, c)).toThrow(/CROSS_SECTOR_WRITE/);
    const same = legal();
    same.sector = "materials";
    same.writes_to = "toy-acoustics";
    expect(validateChild(cat, same).bridges).toHaveLength(3);
  });

  it("BAD_KIND and UNKNOWN_FIELD", () => {
    const k = legal();
    (k as { kind: string }).kind = "blend";
    expect(() => validateChild(cat, k)).toThrow(/BAD_KIND/);
    const u = legal();
    u.parents[1] = { pin: "pin:x", field: "no-such-field" };
    expect(() => validateChild(cat, u)).toThrow(/UNKNOWN_FIELD/);
  });
});
