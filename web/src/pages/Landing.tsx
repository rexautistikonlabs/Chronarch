 ...
 *
 *  Under prefers-reduced-motion, or without WebGL, the campus is not mounted
 *  and the hero and chapters stand as stacked HTML. Continuum is never
 *  mounted inside this app; this page never imports the Chronarch well. */
...
export interface Chapter {
  ...
  source?: { href: string; label: string }; // a secondary source link, new tab
}
...
export const CHAPTERS: readonly Chapter[] = [
  ...
  },
  {
    ...
    name: "Continuum",
    status: "RUNNING",
    sentences: ["Continuum is a simulation; its numbers are model outputs, not measurements of any person.", "It runs at its own address, not inside this app; its source is public."],
    ...
    source: { href: SCIENTIFICLAB_URL, label: "Continuum source" },
  },
  {
    key: "laterion",
    name: "Laterion",
      ...
      <div className={`hud-card pointer-events-auto ${stacked ? "w-full max-w-2xl" : "w-full max-w-md"}`}>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {c.door?.kind === "external" && (
            <a href={c.door.href} onClick={(e) => { e.preventDefault(); onDoor(c.key); }} className="hud-button inline-block" data-testid={`cta-${c.key}`} data-door="external">Open {c.name} ↗</a>
          )}
          {c.source && (
...
function Footer() {
  return (
    <footer className="pointer-events-auto border-t hair px-6 py-6 text-[11px] text-dim" data-testid="landing-footer">
      ...
      <p className="mt-3">
        <span className="readout uppercase tracking-wider">attributions · {ATTRIBUTION_NOTE}</span>{" "}
        {ATTRIBUTIONS.map((a, i) => (
          <span key={a.href}>{i > 0 ? " · " : ""}<a href={a.href} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:text-ivory" data-testid={`footer-attribution-${a.label}`}>{a.label}</a> — {a.what}</span>
        ))}
      </p>
...
export function Landing() {
  ...
  // Where a door leads, once it has opened.
  ...
  const pick = useCallback((k: BuildingKey) => {
    ...
    door.current = k; // the rig eases at the volume while the door opens
    setLeaving(k);
  }, [campus, go, leaving, reduced]);
  ...
  if (gated) return <Gate onAccept={accept} />;
